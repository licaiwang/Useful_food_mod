...
public class usefulfood implements ModInitializer {
    ...
    public static final Item Suspicious_Meat = new FoodBase(new Item.Settings().group(ItemGroup.FOOD).food(new FoodComponent.Builder().hunger(6).saturationModifier(6f).meat().alwaysEdible().statusEffect(new StatusEffectInstance(StatusEffects.NAUSEA,20*30), 0.5f).statusEffect(new StatusEffectInstance(StatusEffects.INSTANT_DAMAGE,10), 0.3f).build()));
    ...
    public static final Item Chew_Bones = new FoodBase(new Item.Settings().group(ItemGroup.FOOD).food(new FoodComponent.Builder().snack().alwaysEdible().statusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 20*60), 0.5f).build()));
